        ...
        try {
            ...
            for (let i = 1; i <= 4; i++) {
                ...
                if (!checkbox || !checkbox.checked) {
                    document.getElementById(`currentVoltage${i}`).textContent = "0.000 [V]";
                    continue;
                }
                ...
                const cmd = createReadCommand(deviceId, '1000');
                const response = await sendCommand(cmd);
                if (response) {
                    const data = parseResponse(response);
                    if (data && checkbox.checked) {  // 再度チェック状態を確認
                        document.getElementById(`currentVoltage${i}`).textContent =
                            (data.value / 1000).toFixed(3) + " [V]";
                    }
                }
            }
        } catch (error) {
